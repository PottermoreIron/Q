...
export default function StrategiesPage() {
  ...
  const loadStrategies = useCallback(() => {
    ...
    api
      ...
      .catch((err: unknown) => {
        setLoadError(formatApiError(err, "Failed to load strategies."));
      })
  ...
  const handleDelete = async (id: string) => {
    try {
      ...
      setLoadError(formatApiError(err, "Failed to delete strategy."));
    }
  };
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <div className="py-24 text-center">
          <ErrorBanner message={loadError} onRetry={loadStrategies} />
        </div>
      ) : items.length === 0 ? (
        <div className="py-24 text-center">
          <p className="font-serif italic text-title text-muted mb-4">
            No strategies yet.
          </p>
          <Link
            href="/strategies/new"
